...
<?php include_partial('twSubscriptionMailQueue/assets') ?>

<div id="sf_admin_container">
  <h1><?php echo __('TwSubscriptionMailQueue List', array(), 'messages') ?></h1>

  <?php include_partial('twSubscriptionMailQueue/flashes') ?>

  <div id="sf_admin_header">
    <?php include_partial('twSubscriptionMailQueue/list_header', array('pager' => $pager)) ?>
  </div>

  <div id="sf_admin_bar">
    <?php include_partial('twSubscriptionMailQueue/filters', array('form' => $filters, 'configuration' => $configuration)) ?>
  </div>

  <div id="sf_admin_content">
    <form action="<?php echo url_for('tw_subscription_mail_queue_collection', array('action' => 'batch')) ?>" method="post">
    <?php include_partial('twSubscriptionMailQueue/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper)) ?>
    <ul class="sf_admin_actions">
      <?php include_partial('twSubscriptionMailQueue/list_batch_actions', array('helper' => $helper)) ?>
      <?php include_partial('twSubscriptionMailQueue/list_actions', array('helper' => $helper)) ?>
    </ul>
    </form>
  </div>

  <div id="sf_admin_footer">
    <?php include_partial('twSubscriptionMailQueue/list_footer', array('pager' => $pager)) ?>
  </div>
</div>
...
<?php slot('contentViews') ?>
      <ul class="contentViews">
        ...
        <li id="contentview-info" class="selected"><?php echo link_to(__('Emails Queue', null, 'subskrypcja'), '@tw_subscription_mail_queue', array('accesskey' => 'l')) ?></li>
